...
data <-  tibble(
  sample_id = c("SRR5223499", "SRR5223500", "SRR5223504", "SRR5223505", 
                "SRR5223521", "SRR5223522", "SRR5223524", "SRR5223542", 
                "SRR5223543", "SRR5223546", "SRR5223547", "SRR5223570"),
  replicate = c("A", "A", "A", "A", "B", "B", "B", "D", "D", "D", "D", "B"),
  time = c("T03", "T04", "T03", "T04", "T03", "T04", "T03", "T03", "T04", "T03", "T04", "T04"),
  treatment = c("G02", "G02", "G03", "G03", "G02", "G02", "G03", "G02", "G02", "G03", "G03", "G03")
)

ExploreModelMatrix(sampleData = data,
...
When you have many variables (es: time point * genotype * treatment) it can get very difficult to extract the precise contrast you want. In these cases, it can be easier to just change the baseline to coincide with one of the two conditions you want to compare, so that setting the final contrasts results easier.

If we do this, it is necessary to repeat the DESeq(dds) step.

Knowing that the "relevel" functionc an change the baseline in a dds object, try to change the baseline for time to T04 and then extract the genes differentially expressed between T04G03 and T04G02


```{r newbaseline, exercise=TRUE, exercise.eval=TRUE, exercise.setup="G03_vs_G02_at_T03"}

```

```{r newbaseline-hint-1}
?relevel
dds <- DESeq(dds)
resultsNames(dds)
?results
...
```{r newbaseline-hint-2}
dds$time <- relevel(dds$time, ref="T04")
dds <- DESeq(dds)
resultsNames(dds)
?results
```

```{r newbaseline-solution}
dds$time <- relevel(dds$time, ref="T04")
dds <- DESeq(dds)
resultsNames(dds)
res <- results(dds, name="treatment_G03_vs_G02")
```
